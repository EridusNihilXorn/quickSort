...
{
	// Shuffle elements of array to maximize effectiveness for bad cases.
	if (left < 0 || right >= size)
	{
		std::cout << "Wrong limits of array elements.\n";
		return;
	} 
...
int partition(T* summable, int left, int right)
{
	...
	while (true)
	{
		while (summable[++i] < summable[left])	// Find element from left that does not fit it's position.
			if (i == right) break;
		while (summable[--j] > summable[left])	// Find element from right that ...
			if (j == left) break;

		if (i >= j) break;  // If pointers cross, no more swaps required.
